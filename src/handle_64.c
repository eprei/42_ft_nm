...
static char decode_section_type (const Elf64_Xword *section_flags, const Elf64_Word *section_type){
...
static char decode_symbol_type(const Elf64_Sym *symbol, const Elf64_Shdr* section_of_symbol) {
    const unsigned char symbol_bind = ELF64_ST_BIND(symbol->st_info);
    const unsigned char symbol_type = ELF64_ST_TYPE(symbol->st_info);
    const Elf64_Word	section_type = section_of_symbol == NULL ? 0 : section_of_symbol->sh_type;
    const Elf64_Xword	section_flags = section_of_symbol == NULL ? 0 : section_of_symbol->sh_flags;
    char type;
...
static void fill_my_symbol(const Elf64_Sym *symbol, t_my_symbol_64 *new_symbol, const Elf64_Shdr* section_of_symbol, char *name){
...
static void swap_symbols(t_my_symbol_64 *first, t_my_symbol_64 *second) {
    t_my_symbol_64 tmp;
...
static void sort_my_symbols(t_my_symbol_64 *symbols_array, long unsigned int symbols_amount, bool reverse_sort) {
...
static Elf64_Shdr *get_section(const Elf64_Ehdr* elf_header, Elf64_Shdr* section_header, const char *table){
    const int shstrtab_index = elf_header->e_shstrndx;
    Elf64_Shdr *shstrtab_hdr = &section_header[shstrtab_index];
...
int handle_64(const Elf64_Ehdr *elf_header, const unsigned long file_size, const char *file_path, t_opts* opts) {
    Elf64_Shdr *symtab_section, *strtab_section, *section_header;
    Elf64_Sym *symbols_array;
    ...
    section_header = (Elf64_Shdr *) ((char *)elf_header + elf_header->e_shoff);
    ...
    my_symbols_array = calloc(sizeof(t_my_symbol_64), symbols_amount);
    if (my_symbols_array == NULL) {
        ft_printf("Error in handle_64: calloc error\n");
        return EXIT_FAILURE;
    }

    // Fill all my symbols
    symbols_array = (Elf64_Sym *)((char *) elf_header + symtab_section->sh_offset);
    char *strtab = (char *) elf_header + strtab_section->sh_offset + 1;
    for (unsigned int i = 0; i < symbols_amount; i++) {
        const Elf64_Sym *symbol = (Elf64_Sym *) (char *) &symbols_array[i];
        const Elf64_Shdr* section_of_symbol = symbol->st_shndx >= elf_header->e_shnum ? 0 : &section_header[symbol->st_shndx];
    ...
    print_my_symbols_64(my_symbols_array, symbols_amount, opts);
